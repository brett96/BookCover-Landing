...
/** Map portal return URLs (including stale *.vercel.app) to the canonical member demo. */
export function resolveMemberDemoReturn(returnUrl: string | null): string | null {
  if (!returnUrl?.trim()) return null;
  try {
    const u = new URL(returnUrl);
    if (u.hostname.endsWith("cercalabs.com") || isVercelHost(u.hostname)) {
      return normalizeMemberDemoUrl(returnUrl);
    }
  } catch {
    /* invalid */
  }
